    ...
    internal static class Program
    {
        private static async Task Main()
        {
            ...
            */
            ...
            // 5. Call AcceptChanges method on data set
            dataSet.AcceptChanges();

            // B. Create observable data set to use it for binding to UI
            IObservableDataSet observableDataSet = new ObservableDataSet(dataSet);

            // C. Do modifications to data in observable data set to simulate user input
            // 6. Add row to Employee table to have one row in Added state
            IObservableDataRow employeeObservableDataRow = observableDataSet.Tables["Employee"].AddNewRow();
