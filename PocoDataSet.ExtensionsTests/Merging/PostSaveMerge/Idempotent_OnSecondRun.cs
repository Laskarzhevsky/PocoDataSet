...
{
    /// <summary>
    /// PostSave hardening: idempotence. Running PostSave twice with the same changeset must produce
    ...
    public partial class PostSaveMerge
    {
        ...
        public void Idempotent_OnSecondRun()
        {
            ...
            // Act
            // first PostSave applies saved values and accepts changes in current.
            ...
            // Assert
            // first run applied values and accepted changes.
            ...
            // Act
            // run PostSave again with the same changeset.
            MergeOptions secondOptions = new MergeOptions();
            // Execute PostSave merge: apply server-returned changes (Added/Modified/Deleted) onto current rows.
            current.DoPostSaveMerge(changeset, secondOptions);

            // Assert
            // second run must be a no-op (no merge result entries).
