...
namespace PocoDataSet.ExtensionsTests
{
    public partial class DataRowExtensionsTests
    {
        ...
        public void MergeWith_RowInDeletedState()
        {
            ...
            // Act
            // 5. Call MergeWith method and observe that merge is not done because the  row in Deleted state
            departmentDataRow.MergeWith(departmentRefreshedDataRow, "Department", departmentDataTable.Columns, new MergeOptions());

            // Assert
            Assert.Equal(DataRowState.Deleted.ToString(), departmentDataRow.DataRowState.ToString());
            Assert.Equal("Sales", departmentDataRow["Name"]);
        }
    }
}
