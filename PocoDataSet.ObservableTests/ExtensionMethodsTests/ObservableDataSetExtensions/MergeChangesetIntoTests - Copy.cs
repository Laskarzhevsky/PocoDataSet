...
namespace PocoDataSet.ObservableTests.ObservableDataSetExtensions
{
    ...
    {
        [Fact]
        public void MergeChangesetInto()
        {
            // Arrange
            ...
            // 5. Create an empty data set for the merge result
            IDataSet mergeResultDataSet = DataSetFactory.CreateDataSet();


            // Act
            // 6. Call MergeChangesetInto method tomerge changeset into the merge result
            observableDataSet.MergeChangesetInto(mergeResultDataSet);

            // Assert
            // All tables have one row only and the value of Id field of every row is 2
            Assert.Equal(1, mergeResultDataSet.Tables["Department"].Rows.Count);
            Assert.Equal(2, mergeResultDataSet.GetFieldValue<int>("Department", 0, "Id"));

            Assert.Equal(1, mergeResultDataSet.Tables["EmploymentType"].Rows.Count);
            Assert.Equal(2, mergeResultDataSet.GetFieldValue<int>("EmploymentType", 0, "Id"));

            Assert.Equal(1, mergeResultDataSet.Tables["Employee"].Rows.Count);
            Assert.Equal(2, mergeResultDataSet.GetFieldValue<int>("Employee", 0, "Id"));
        }
    }
}
