...
namespace PocoDataSet.ObservableTests.ObservableDataSetExtensions
{
    public partial class ObservableDataSetExtensionsTests
    {
        ...
        public void CreateChangeset()
        {
            ...
            Assert.Equal(1, changeset.Tables["Department"].Rows.Count);
            Assert.Equal(2, changeset.GetFieldValue<int>("Department", 0, "Id"));

            Assert.Equal(1, changeset.Tables["EmploymentType"].Rows.Count);
            Assert.Equal(2, changeset.GetFieldValue<int>("EmploymentType", 0, "Id"));

            Assert.Equal(1, changeset.Tables["Employee"].Rows.Count);
