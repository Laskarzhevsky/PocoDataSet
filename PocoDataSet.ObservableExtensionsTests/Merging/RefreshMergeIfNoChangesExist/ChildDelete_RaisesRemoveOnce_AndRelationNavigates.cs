...
public partial class RefreshMergeIfNoChangesExist
{
    ...
    ///
    /// Scenario:
    /// - CURRENT has 1 Customer and 2 Orders (children) for that customer.
    /// - REFRESHED snapshot includes only 1 of the Orders.
    ///
    /// Expectations:
    /// - Orders table raises exactly one CollectionChanged(Remove) event (and no Add).
    /// - Immediately after merge, relation navigation reflects the removal
    ///   (GetChildRows returns 1 order for the customer).
    /// </summary>
    [Fact]
    public void ChildDelete_RaisesRemoveOnce_AndRelationNavigates()
    {
        ...
        // Arrange REFRESHED snapshot missing OrderId=11.
        ...
        // Assert: notification contract (exactly one Remove, no Add).
        Assert.Equal(0, counter.AddEvents);
        Assert.Equal(1, counter.RemoveEvents);

        // Assert: relation navigation reflects the removal immediately after merge.
