...
{
    /// <summary>
    /// Ensures observable RefreshMergeIfNoChangesExist keeps relations consistent AND raises exactly one
    /// collection Add notification when a child row is introduced by the refreshed snapshot.
    ///
    /// Scenario:
    /// - CURRENT has 1 Customer and 1 Order (child) referencing that customer via relation CustomerOrders.
    /// - REFRESHED snapshot has the same Customer and adds one more Order for the same Customer.
    ///
    /// Expectations:
    /// - Orders table raises exactly one CollectionChanged(Add) event (and no Remove).
    /// - Immediately after merge, relation navigation reflects the new child row
    ///   (GetChildRows returns 2 orders for the customer).
    /// </summary>
    [Fact]
    public void ChildAdd_RaisesAddOnce_AndRelationNavigates()
    {
        ...
        // Arrange REFRESHED snapshot with one new child row.
        ...
        // Assert: notification contract (exactly one Add, no Remove).
        Assert.Equal(1, counter.AddEvents);
        Assert.Equal(0, counter.RemoveEvents);

        // Assert: relation navigation reflects the new row immediately after merge.
