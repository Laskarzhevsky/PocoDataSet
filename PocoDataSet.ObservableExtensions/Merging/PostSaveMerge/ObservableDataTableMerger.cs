...
#region Keyed Merge
            ...
            bool requiresClientKeyCorrelation)
        {
            for (int i = currentObservableDataTable.Rows.Count - 1; i >= 0; i--)
            {
                ...
                {
                    // PostSave contract hardening:
                    // - If the server returns a row in Unchanged state, we treat it as a "save echo".
                    // - We allow it to reconcile client Added rows (server-assigned identity / computed columns).
                    // - We MUST NOT allow it to overwrite client Modified/Unchanged rows (prevents silent data loss).
                    if (isPostSaveMerge
                        && refreshedDataRow.DataRowState == DataRowState.Unchanged
                        && observableDataRow.InnerDataRow.DataRowState != DataRowState.Added)
                    {
                        // Still consider the refreshed row processed so it is not added as a new row later.
                        string echoedPk;
                        bool hasEchoedPk = RowIdentityResolver.TryGetPrimaryKeyValue(refreshedDataRow, primaryKeyColumnNames, out echoedPk);
                        if (!hasEchoedPk)
                        {
                            echoedPk = string.Empty;
                        }

                        processedRefreshedPrimaryKeys.Add(echoedPk);
                        continue;
                    }
